...
const Reader: React.FC<ReaderProps> = ({
  ...
  useEffect(() => {
    if (loading || error || !chapterContent) return;
    
    const newChunks: TextChunk[] = chapterContent.map((segment, index) => ({
      id: index,
      hebrew: segment.hebrew,
      translation: segment.english,
      isLoading: false,
      context: segment.hebrew,
    ...
    const items: MinimapItem[] = chapterContent.map((segment, index) => ({
      title: `Section ${segment.seq}`,
      index
    }));
    ...
    const markedContext = newChunks.map(c => `[Section ${c.sequence}]: ${c.hebrew}`).join('\n\n');
    onContextUpdate(markedContext);
  }, [chapterContent, loading, error, onContextUpdate]);
  ...
  const handleGenerateTranslation = async (chunkIndex: number) => {
    ...
    if (!targetChunk.context || !currentBookId) return;
      ...
      setChunks(prev => prev.map((c, i) => {
        ...
        if (segIdx !== -1 && translations[segIdx]) {
          ...
          if (userId && currentChapterId) {
            supabase.from('segments').upsert({
              id: c.id,
              chapter_id: currentChapterId,
              hebrew_text: c.hebrew,
              english_translation: translations[segIdx],
          ...
          return {
            ...c,
            translation: translations[segIdx],
            isLoading: false
          };
        }
        return c;
  ...
  const handleSelection = useCallback(() => {
    ...
    if (sel && sel.toString().trim().length > 0) {
      ...
      setSelection({ text: sel.toString(), rect: rect });
    } else {
      setSelection(null);
    }
  }, []);
  ...
  if (loading) {
    return (
      <main className={`flex-1 h-full relative transition-colors duration-500 ${THEME_CLASSES[settings.theme]}`}>
        <div className="h-full flex items-center justify-center">
          <p className="text-lg font-serif opacity-50">Loading chapter content...</p>
        </div>
      </main>
    );
  }
  ...
  if (error) {
    return (
      <main className={`flex-1 h-full relative transition-colors duration-500 ${THEME_CLASSES[settings.theme]}`}>
        <div className="h-full flex items-center justify-center">
          <p className="text-lg font-serif text-red-500">Error loading chapter: {error}</p>
        </div>
      </main>
    );
  }
    ...
    >
      ...
      >
        ...
        <div className="min-h-screen flex flex-col items-center justify-center p-12 text-center">
          <h1 className={`font-bold opacity-80 tracking-wide leading-tight mb-8 ${isHebrew(title) ? 'font-hebrew-serif' : 'font-serif'}`}
            ...
            >
              ...
              <div className="w-full max-w-5xl flex flex-col items-center gap-12">
                ...
                {settings.translationMode !== 'english' && (
                  <p 
                    dir="rtl" 
                    className={`font-hebrew-serif ${getAlignClass('hebrew')} leading-loose transition-all duration-500 selection:bg-gray-200 selection:text-black dark:selection:bg-gray-800 dark:selection:text-white max-w-2xl w-full mx-auto`}
      ...
      {selection && (
        ...
        >
          ...
          <div className={`shadow-2xl rounded-2xl p-2 flex items-center gap-2 border bg-white text-gray-900 border-gray-200 dark:bg-gray-800 dark:text-gray-100 dark:border-gray-700 whitespace-nowrap`}>
            <button 
              onClick={(e) => {
                e.stopPropagation();
